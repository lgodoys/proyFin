...
class Espacios(tk.Toplevel):
    def __init__(self,parent):
        ...
        ttk.Label(self,text="Indique la placa del vehiculo autorizado").place(x=20,y=160)             
        placa_entry= ttk.Entry(self,textvariable=placa)
        placa_entry.place(x=20, y=190)
        ...
        espacio = partial(self.RegistroEspacio, idespacio,placa,direccion)

        b1=ttk.Button(self, text = "Registrar", command = espacio, style="C.TButton")
        b1.place(x = 60, y = 300)

    def RegistroEspacio(self,idespacio,placa,direccion):
        respuestaMySQL, estado, errorMySQL = registroEspacio(idespacio.get(),placa.get(),direccion.get())
